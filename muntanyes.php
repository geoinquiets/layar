...
//
function Gethotspots( $db, $value, $table = "ign", $reverse = false ) {
...
$sign = $reverse ? ">" : "<";
  ...
  $sql = $db->prepare( "
    		...
    		FROM ".$table.
    		" HAVING distance ".$sign." :radius
    		ORDER BY distance ASC
    		LIMIT 0, 60 " ); // limit changed to 60
...
// we get non-mandatory parameters separately
$profile = $_GET["RADIOLIST"];
$faraway = $_GET["CHECKBOXLIST"] ? true : false;
...
try {
	...
	*/
	...
	// Use Gethotspots() function to retrieve POIs within the search range.  
	$response["hotspots"] = Gethotspots( $db, $value, $table, $faraway );
